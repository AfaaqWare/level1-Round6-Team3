...
function ResponseHeader({ myResponses }: { myResponses: AllResponses[] }) {
  const t = useTranslations("dashboard.responses");

  const handleExport = async () => {
    const exportData = (myResponses ?? []).map((response, index) => ({
      ID: index + 1,
      Respondent: response.respondentName,
      Email: response.respondentEmail,
      "Survey ID": response.surveyId,
      Answers: Object.values(response.answers)
        .map(answer => (typeof answer === "object" ? JSON.stringify(answer) : String(answer)))
        .join(" | "),
